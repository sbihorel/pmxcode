# Changelog

## pmxcode 0.3.3

- Various fixes for deployment to Posit Cloud Connect

## pmxcode 0.3.2

CRAN release: 2026-09-20

- Add Beal’s M4 method (ie output predictions for BLQ records)

## pmxcode 0.3.1

CRAN release: 2025-09-07

- Add PREDV variable for Beal’s M3 method (ie output predictions for BLQ
  records)

## pmxcode 0.2.2

- Add feature to fix variability estimates
- Fix issue in parameterization for log and logit scale
- Clean up documentation

## pmxcode 0.2.1

- Change parameterization of logit model for consistency across scales /
  mu transformation
- Add list of parameters in headers
- Fix description issue in headers for biophase model

## pmxcode 0.2.0

CRAN release: 2025-03-01

- Allow users to define parameter on linear, log, or logit scale
- For mrgsolve models, allow users to switch between random variability
  and posthoc estimates of variability
- UI refactoring using bslib
- Fixes typos in M3 method code

## pmxcode 0.1.4

CRAN release: 2024-10-19

- Bug fixes in M3 method coding and app crash with certain model
  combination.

## pmxcode 0.1.3

CRAN release: 2024-06-13

- Bug fixes with target-mediated elimination

## pmxcode 0.1.2

CRAN release: 2024-02-18

- Bug fixes with logit variability model
- Update UI to bslib

## pmxcode 0.1.1

CRAN release: 2023-05-19

- Bug fixes in transit compartment absorption model code for NONMEM
- Bug fixes for default NONMEM \$TABLE variables

## pmxcode 0.1

CRAN release: 2023-04-13

- First release supporting creation of new models for NONMEM and
  mrgsolve
